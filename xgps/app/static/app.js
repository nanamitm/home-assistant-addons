...
const messages = {
  en:{starting:"Starting…",grid:"Grid",projection:"Projection",units:"Units",prnLabels:"PRN labels",resetView:"Reset view",skyView:"Sky view",position:"Position",satellites:"Satellites",elevation:"El",azimuth:"Az",used:"Used",rawJson:"Raw JSON",noSatellites:"No satellite data",yes:"Yes",no:"No",fix:"Fix",noFix:"No fix",latitude:"Latitude",longitude:"Longitude",altitude:"Altitude",speed:"Speed",track:"Track",time:"Time",waiting:"Waiting for gpsd…",uiDisconnected:"Web UI disconnected — retrying…"},
  ja:{starting:"起動中…",grid:"グリッド",projection:"投影",units:"単位",prnLabels:"PRNラベル",resetView:"表示をリセット",skyView:"スカイビュー",position:"測位情報",satellites:"衛星",elevation:"仰角",azimuth:"方位",used:"使用中",rawJson:"Raw JSON",noSatellites:"衛星データがありません",yes:"はい",no:"いいえ",fix:"測位",noFix:"未測位",latitude:"緯度",longitude:"経度",altitude:"高度",speed:"速度",track:"進行方向",time:"時刻",waiting:"gpsdを待機中…",uiDisconnected:"Web UI切断 — 再接続中…"}
};
...
function renderPosition(){const root=$("position");root.replaceChildren();const altitude=tpv.altHAE??tpv.altMSL??tpv.alt,[alt,altUnit]=convertAltitude(altitude),[speed,speedUnit]=convertSpeed(tpv.speed);const rows=[[msg.fix,({1:msg.noFix,2:"2D",3:"3D"})[tpv.mode]||"—"],[msg.latitude,finite(tpv.lat)?`${tpv.lat.toFixed(7)}°`:"—"],[msg.longitude,finite(tpv.lon)?`${tpv.lon.toFixed(7)}°`:"—"],[msg.altitude,`${alt} ${altUnit}`.trim()],[msg.speed,`${speed} ${speedUnit}`.trim()],[msg.track,finite(tpv.track)?`${tpv.track.toFixed(1)}°`:"—"],[msg.time,tpv.time||"—"]];rows.forEach(([key,value])=>{const dt=document.createElement("dt"),dd=document.createElement("dd");dt.textContent=key;dd.textContent=value;root.append(dt,dd)})}
...
function connect(){const ws=new WebSocket(websocketUrl());ws.onopen=()=>setStatus(false,msg.waiting);ws.onmessage=event=>{const data=JSON.parse(event.data);if(data.type==="snapshot"){satellites=data.satellites||[];tpv=data.tpv||{};setStatus(data.connected,data.status);$("raw-card").hidden=!data.rawEnabled;(data.raw||[]).forEach(addRaw);renderSatellites();renderPosition()}else if(data.type==="status"){setStatus(data.connected,data.status)}else if(data.type==="sky"){satellites=data.satellites||[];renderSatellites()}else if(data.type==="tpv"){tpv=data.tpv||{};renderPosition()}else if(data.type==="raw")addRaw(data.line)};ws.onclose=()=>{setStatus(false,msg.uiDisconnected);setTimeout(connect,2000)}}
